...
#Elegir la fecha de los ultimos 3 dias disponibles
#Descarga de "datos ingresados por empresas/brokers" y "precios chicago"
...
comprador= total %>% filter(dist_comprador>2) %>% 
  select(fecha, producto, empresa, posicion_mes, posicion_ano, p_comprador, media_comprador, dist_comprador)
...
vendedor= total %>% filter(dist_vendedor>2)%>% 
  select(fecha, producto, empresa, posicion_mes, posicion_ano, p_vendedor, media_vendedor, dist_vendedor)
...
### empresas y brokers que han reportado precios altos los ultimos 3 dias 

# vendedor_notificar <- bs %>% group_by( producto, empresa, posicion_ano,posicion_mes) %>%
#   summarise(dias = n()) %>% filter(dias == 3) 
# 
# comprador_notificar <- as %>% group_by( producto, empresa, posicion_ano,posicion_mes) %>%
#   summarise(dias = n()) %>% filter(dias == 3)


# library("writexl")
# write_xlsx(vendedor,"U://Precios FOB//vendedor.xlsx")
